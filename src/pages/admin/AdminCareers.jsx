...
import { Plus, Pencil, Trash2, X, ToggleLeft, ToggleRight, Briefcase, MapPin, Clock } from 'lucide-react';

const TYPES = ['full-time', 'part-time', 'contract', 'internship', 'freelance'];
const EMPTY = { title: '', department: '', location: '', type: 'full-time', experience: '', salary: '', description: '', requirements: '', responsibilities: '', benefits: '', applicationEmail: 'careers@webmobhut.com', isActive: true, isFeatured: false };

const TypeBadge = ({ type }) => {
  const colors = { 'full-time': 'bg-green-500/10 text-green-400', 'part-time': 'bg-blue-500/10 text-blue-400', contract: 'bg-orange-500/10 text-orange-400', internship: 'bg-purple-500/10 text-purple-400', freelance: 'bg-pink-500/10 text-pink-400' };
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {careers.map((c) => (
            <div key={c._id} className={`bg-[#0f0f0f] border rounded-xl p-4 transition-all ${c.isActive ? 'border-[#1e1e1e] hover:border-[#2a2a2a]' : 'border-[#1e1e1e] opacity-60'}`}>
